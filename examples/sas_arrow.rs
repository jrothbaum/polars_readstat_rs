...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: sas_arrow <file> [mode] [threads] [chunk_size] [missing_string_as_null] [batch_size]");
        std::process::exit(1);
    }
    ...
    let missing_string_as_null = args
        ...
        .map(|s| s == "true" || s == "1")
        .unwrap_or(true);
    let batch_size = args.get(6).and_then(|s| s.parse::<usize>().ok()).unwrap_or(10000);

    match mode {
        "schema" => {
            let schema = read_to_arrow_schema_ffi(&path, threads, missing_string_as_null, chunk_size)
                .expect("arrow schema");
