...
pub fn sas_files() -> Vec<PathBuf> {
    let base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("sas").join("data");
    collect_files(&base, &["sas7bdat"])
}

pub fn stata_files() -> Vec<PathBuf> {
    let base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("stata").join("data");
    collect_files(&base, &["dta"])
}

pub fn spss_files() -> Vec<PathBuf> {
    let base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("spss").join("data");
