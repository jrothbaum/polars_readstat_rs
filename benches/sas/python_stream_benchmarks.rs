...
fn stream_sas_python_like(
    ...
    n_rows: Option<usize>,
    _window_chunks: usize,
) -> Result<usize, String> {
    let meta_reader = Sas7bdatReader::open(path).map_err(|e| e.to_string())?;
    let total_rows = n_rows
        .unwrap_or(meta_reader.metadata().row_count)
        .min(meta_reader.metadata().row_count);
    ...
    for (start_chunk, end_chunk) in worker_ranges {
        ...
        let handle = std::thread::spawn(move || {
            ...
            let worker_rows = min(total_rows - start_row, (end_chunk - start_chunk) * batch_size);
            let worker_df = match reader
                .read()
