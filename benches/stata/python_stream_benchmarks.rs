...
fn find_test_files() -> Vec<PathBuf> {
    let pattern = format!("{}/tests/stata/data/**/*.dta", env!("CARGO_MANIFEST_DIR"));
    glob(&pattern)
...
fn stream_stata_python_like(
    ...
    let meta_reader = StataReader::open(path).map_err(|e| e.to_string())?;
    ...
    for (start_chunk, end_chunk) in worker_ranges {
        ...
        let handle = std::thread::spawn(move || {
            let reader = match StataReader::open(&path) {
                Ok(r) => r,
                Err(e) => {
...
fn bench_python_stream_large_file(c: &mut Criterion) {
    if let Some(file) = find_test_file("usa_00009") {
        c.bench_function("python_stream_large_file", |b| {
            b.iter(|| {
                let rows = stream_stata_python_like(
                    black_box(&file),
                    black_box(100_000),
